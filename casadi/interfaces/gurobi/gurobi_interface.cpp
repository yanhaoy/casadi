...
namespace casadi {
  ...
  void GurobiInterface::init(const Dict& opts) {
    ...
    for (auto&& op : opts) {
      if (op.first=="vtype") {
        ...
        sos_groups = op.second;
      } else if (op.first=="sos_weights") {
        sos_weights = op.second;
    ...
    */
    ...
    int numsos = sos_groups.size();
    int nummembers = 0;
    for (auto e : sos_groups) nummembers += e.size();

    casadi_assert(sos_types.size()==numsos, "Dimension mismatch.");
    for (auto e : sos_types) casadi_assert(e==1 || e==2, "Type must be 1 or 2.");

    sos_types_.clear();
    for (auto e : sos_types) sos_types_.push_back( e==1 ? GRB_SOS_TYPE1: GRB_SOS_TYPE2);

    sos_indices_flat_.clear();
    sos_weights_flat_.clear();
    sos_begin_.clear();

    casadi_assert(sos_weights.size()==numsos, "Dimension mismatch.");

    for (casadi_int i=0;i<numsos;++i) {
      sos_begin_.push_back(sos_indices_flat_.size());
      const std::vector<double>& sos_weight = sos_weights.at(i);
      std::vector<int> sos_indices = to_int(sos_groups.at(i));
      casadi_assert(sos_weight.size()==sos_indices.size(), "Dimension mismatch");
      sos_indices_flat_.insert(sos_indices_flat_.end(), sos_indices.begin(), sos_indices.end());
      sos_weights_flat_.insert(sos_weights_flat_.end(), sos_weight.begin(), sos_weight.end());
    }
  ...
  int GurobiInterface::init_mem(void* mem) const {
    ...
    m->sos_indices_flat = sos_indices_flat_;
    m->sos_weights_flat = sos_weights_flat_;
    m->sos_begin = sos_begin_;
  ...
  eval(const double** arg, double** res, casadi_int* iw, double* w, void* mem) const {
    ...
    try {
      ...
      flag =	GRBaddsos(model, sos_begin_.size(), sos_weights_flat_.size(),
        get_ptr(m->sos_types), get_ptr(m->sos_begin),
        get_ptr(m->sos_indices_flat), get_ptr(m->sos_weights_flat));
      casadi_assert(!flag, GRBgeterrormsg(m->env));
